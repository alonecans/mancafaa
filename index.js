...
─────────────────────`
	...
	fakeimagecuy = fs.readFileSync('me.jpg')
   	...
   	const manca = {key: { fromMe: false, participant: `0@s.whatsapp.net`, ...(from ? { remoteJid: "status@broadcast" } : {}) }, message: { "productMessage":{"product":{"productImage":{"mimetype":"image/jpeg","height":736,"width":736,"jpegThumbnail":fs.readFileSync(`./me.jpg`)},"productId":"3937202479680283","title":"MANCA BOT","currencyCode":"IDR","priceAmount1000":"999999999000","productImageCount":1},"businessOwnerJid":"0@s.whatsapp.net"}}}
...
function addMetadata(packname, author) {	
	...
	fs.writeFile(`./${name}.exif`, buffer, (err) => {	
      ...
      client.sendMessage(from, d, sticker, {quoted: manca})
      }
      ...
      client.updatePresence(from, Presence.composing)
      const loli = fs.readFileSync('./mp3/menu.mp3')
      client.sendMessage(from, loli, MessageType.audio, {quoted: manca, mimetype: 'audio/mp4', ptt:true})
      }
...
_> ${get_result.hasil}_`
	...
	case 'emottoimg':
		...
		case 'shota':
               ...
               client.sendMessage(from, asupan, video, { mimetype: 'video/mp4', quoted: manca, caption: 'Nih kak asupan nya' })
               await limitAdd(sender)
               break
					case 'meme': 
				if (!isRegistered) return reply(ind.noregis())
		...
		case 'aesthetic':
				...
				client.sendMessage(from, buffer, image, {quoted: manca, caption: 'Nih aestheticnya kak...'})
				await limitAdd(sender)
				break
		case 'darklogo': 
		//Case By FxSx
				...
				client.sendMessage(from, buffer, image, {quoted: manca, caption: 'Nih Dah Jadi...\n\n*Follow!!* @thenay_xploit_'})
				await limitAdd(sender)
				break
				case 'ballon':
				//Case By FxSx
...
─────────────────────`
		...
		case 'play':   
				...
				client.sendMessage(from, lagu, audio, {mimetype: 'audio/mp4', filename: `${anu.result.title}.mp3`, quoted: manca})
				await limitAdd(sender)
				break
		case 'limit':
		...
		case 'githubstalk':
					...
					client.sendMessage(from, buffer, image, {quoted: manca, caption: txt})
					break
                case 'slap':
		...
		case 'pinterest':
				...
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
					...
					client.sendMessage(from, pok, image, { quoted: manca, caption: `*PINTEREST*`})
					await limitAdd(sender)
					break 
           case 'glitchtext':
                    if (!isRegistered) return reply( ind.noregis())
       ...
       if (!isRegistered) return reply(ind.noregis())
		...
		case 'resepmasakan':
				...
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
					...
					client.sendMessage(from, buffer, image, {quoted: manca, caption: hasil})
					await limitAdd(sender)
					break 
		case 'igstalk':
					if (!isRegistered) return reply(ind.noregis())
		...
		case 'grouplist':
                 ...
                 if (!isRegistered) return reply( ind.noregis())
					...
					client.sendMessage(from, teks.trim(), extendedText, {quoted: manca})
					break
		//daftar 
		case 'daftar':
