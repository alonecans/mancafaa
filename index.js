	...
	if (!welkom.includes(anu.jid)) return
		try {
			...
			if (anu.action == 'add') {
				...
				client.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}, quoted: { key: { fromMe: false, participant: `0@s.whatsapp.net`, ...(from ? { remoteJid: "status@broadcast" } : {}) }, message: { "productMessage":{"product":{"productImage":{"mimetype":"image/jpeg","height":736,"width":736,"jpegThumbnail":fs.readFileSync(`./me.jpg`)},"productId":"3937202479680283","title":"HAI SELAMAT DATANG","currencyCode":"IDR","priceAmount1000":"999999999000","productImageCount":1},"businessOwnerJid":"0@s.whatsapp.net"}}}})
				...
				let buff = await getBuffer(ppimg)
				client.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}, quoted: { key: { fromMe: false, participant: `0@s.whatsapp.net`, ...(from ? { remoteJid: "status@broadcast" } : {}) }, message: { "productMessage":{"product":{"productImage":{"mimetype":"image/jpeg","height":736,"width":736,"jpegThumbnail":fs.readFileSync(`./me.jpg`)},"productId":"3937202479680283","title":"BEBAN GRUP KELUAR","currencyCode":"IDR","priceAmount1000":"999999999000","productImageCount":1},"businessOwnerJid":"0@s.whatsapp.net"}}}})
			}
		} catch (e) {
		console.log(e)
	}
...
_> ${get_result.hasil}_`
			...
			case 'slow':
					...
					exec(`ffmpeg -i ${media} -filter:a "atempo=0.7,asetrate=44100" ${ran}`, (err, stderr, stdout) => {
						...
						if (err) return reply('Error!')
						hah = fs.readFileSync(ran)
						client.sendMessage(from, hah, audio, {mimetype: 'audio/mp4', ptt:true, quoted: mek2})
						fs.unlinkSync(ran)
					})
				break
				case 'gemuk':
              ...
              case 'tupai':
				...
				client.sendMessage(from, uhh, audio, {mimetype: 'audio/mp4', ptt:true, quoted: mek2})
				fs.unlinkSync(ran)
				})
				break
				case 'addbadword': 
	...
	break
       ...
       case 'bann':
				...
				client.sendMessage(from, `Berhasil membanned nomor : wa.me/${bnnd}`, text, {quoted: mek2})
				break
		case 'unbann':
				if (!isAdmin) return reply('*Only Admin bot*')
				bnnd = body.slice(7)
				...
				fs.writeFileSync('./database/pengguna/banned.json', JSON.stringify(ban))
				client.sendMessage(from, `Nomor wa.me/${bnnd} telah di unban!`, text, {quoted: mek2})
				break
        case 'ban':
				if (!isOwner) return reply(ind.ownerb())
				bnnd = body.slice(5)
